...
menu_entry_t *menuhit(XButtonEvent * e)
{
	XEvent ev;
	...
	dx = 0;
	 ...
	 wide = XTextWidth(font, walker->name, strlen(walker->name)) +4;
	 if(wide > dx) dx = wide;

	 walker = (menu_entry_t *)walker->node.next;
	 if(walker == menu_list) break;
	}
        /*
	for (n = 0; m->item[n]; n++) {
	...
	*/
	...
	for (;;) {
		...
		case ButtonRelease:
			...
			walker = menu_list;
			for(x = 0;x!=i;x++) {
			 walker = (menu_entry_t *)walker->node.prev; 
			} 
			return walker;
		...
		case Expose: 
		 ...
		 // else
		  ...
		  XDrawString(dpy, s->menuwin, s->gc, tx, ty, item, 
				  strlen(item));
		  walker = (menu_entry_t *)walker->node.prev;
		 }
		 if(cur>= 0 && cur < n)
		  XFillRectangle(dpy, s->menuwin, s->gc, 0, 
				 cur*high, wide, high);
		 drawn = 1;
